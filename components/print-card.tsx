  ...
  <div className="relative bg-[url('/Sejatizen Card-back.png')] w-[1012px] h-[637px] justify-center card-back">
    ...
    <div className="absolute w-[1012px] mx-auto h-[318.5px] box-content top-[130px] grid grid-cols-2 gap-x-[230px]">
      <div className='block ml-[250px]'>
        <p className={cn(inconsolata.className, "w-[293px] text-white text-[62px] text-center font-normal")}>{memberId}</p>
        <p className={cn(openSans.className, "mt-[30px] w-[293px] text-black text-[38px] text-center font-semibold")}>{name}</p>
        <p className={cn(openSans.className, "mt-[45px] w-[293px] text-black text-[38px] text-center font-semibold")}>{joinDate}</p>
      </div>

      <QRCode className="relative top-[35px] mx-auto" size={250} level='Q' fgColor='white' bgColor='transparent' value={memberId} />
    </div>
  </div>
);

// print button
const PrintButton = ({ componentRef, ...props }: { componentRef: RefObject<HTMLDivElement> }) => {
  ...
  return (
    <Button className={cn(roboto.className, "mx-auto lap:rounded-full tab:rounded-[20px] mob:rounded-[20px] lap:h-11 tab:h-12 mob:h-14 text-s text-[#000123] lap:w-[40%] tab:w-[60%] mob:w-[70%] bg-[#3C739D] hover:text-white hover:bg-[rgb(60,115,157,0.5)] active:text-white active:bg-[rgba(60,115,157,0.7)] mutating:text-[#000123] mutating:bg-[#3C739D] mutating:cursor-progress")} onClick={handlePrint} disabled={isMutating} {...props}>
      {text}
    </Button>
  );
};

const MemberCard = ({ componentRef }: { componentRef: RefObject<HTMLDivElement> }) => {
  return (
    <div ref={componentRef} className='absolute -z-50 -top-[200vh] -left-[200vw]'>
